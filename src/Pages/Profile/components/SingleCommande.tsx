  ...
  return (
    <div className='border border-black rounded mt-8 max-sm:m-2 max-sm:mt-8'>
        ...
        <div className='flex justify-between p-4 max-sm:p-2 border-black border-b-2 bg-black '>
            <p className='text-white max-sm:text-sm max-sm:w-[200px]'>{element.status_paiement === 'unpaid' ? "Tentative d'achat le:" : "Commande effectuée le:"} {element.date_creation_commande}</p>
            <p className='text-white max-sm:text-sm max-sm:w-[100px]'>Total: {element.prixTotal}.00 €</p>
        </div>
        <div className='p-4 max-sm:p-2'>
            <div className='flex items-center justify-between'>
                <p className='max-sm:text-sm'>{element.articles.length} produits commandé{element.articles.length > 1 ? 's' : ''}</p>
                <p className='max-sm:text-sm uppercase'>paiement: {paymentMethod}</p>
            </div>

            {
                element.articles.map((product:any, index:number) => {
                    return (
